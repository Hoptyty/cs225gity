...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() > 1) {
        TreeNode* left_child = removeSmallest(singleQueue, mergeQueue);
        TreeNode* right_child = removeSmallest(singleQueue, mergeQueue);
        TreeNode* internal_node = new TreeNode(left_child -> freq.getFrequency() + right_child -> freq.getFrequency());
