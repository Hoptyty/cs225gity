...
class BinaryTree
{
    ...
    private:
        ...
        /**
         * Tianyu - User created helper function
         *
         */
        void mirror(Node* subRoot);
        bool isOrderedRecursive(const Node* subRoot) const;
        bool isOrderedRecursive(const Node* subRoot, T& largest) const;
        T findlargest(Node* subRoot) const;
        T findsmallest(Node* subRoot) const;
        void getPaths(vector<vector<T>>& paths, vector<T>& singlePath, const Node* subRoot) const;
        int sumDistances(const Node* subRoot, int offset) const;
