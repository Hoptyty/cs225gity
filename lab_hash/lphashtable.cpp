...
void LPHashTable<K, V>::resizeTable()
{
    ...
    delete []should_probe;
    bool* new_probe = new bool[new_size];
    for (size_t i = 0; i < size; i++) {
        if (table[i] != NULL) {
            size_t idx = hash(table[i] -> first, new_size);
            for (size_t j = 0; j < new_size; j++) {
                if (new_table[idx] == NULL && !new_probe[idx]) {
                    new_table[idx] = new pair<K, V>(table[i] -> first, table[i] -> second);
                    new_probe[idx] = true;
                    break;
                }
                idx = (idx + 1) % new_size;
