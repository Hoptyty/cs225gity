...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    new_right -> elements.assign(child -> elements.begin() + mid_elem_idx + 1, child -> elements.end());
    new_right -> children.assign(child -> children.begin() + mid_child_idx, child -> children.end());
    new_left -> elements.assign(child -> elements.begin(), child -> elements.begin() + mid_elem_idx);
    new_left -> children.assign(child -> children.begin(), child -> children.begin() + mid_child_idx);
