...
class KDTree
{
  ...
  private:
     ...
     */
    void findNearestNeighbor(KDTreeNode* node, const Point<Dim>& query, int dim, double& radius_sqr, KDTreeNode*& curr_best) const;
    KDTreeNode* copy(KDTreeNode* node);
    void clear(KDTreeNode* node);
    KDTreeNode* buildTree(vector<Point<Dim>>& kdtree, int start, int end, int dim);
    void quickSelect(vector<Point<Dim>>& kdtree, int start, int end, int dim);
};
